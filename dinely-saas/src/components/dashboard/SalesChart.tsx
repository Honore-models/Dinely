...
export function SalesChart({ revenueChart, loading }: SalesChartProps) {
  // Fallback to placeholder data when empty
  const data =
    ...
    <DashboardCard className="h-full">
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto overflow-y-visible">
          ...
          >
            ...
            <path d={linePath} fill="none" stroke="url(#salesLine)" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" />
      ...
      <div className="mt-3 flex flex-wrap gap-4 border-t border-neutral-100 pt-4">
        <div className="text-xs font-semibold text-neutral-500">
          ...
          <span className="text-neutral-800">{data[Math.max(peakIndex, 0)]?.day ?? "—"}</span>
        </div>
        <div className="text-xs font-semibold text-neutral-500">
          <span className="text-neutral-400">Total</span>{" "}
          <span className="text-[#22c51f]">{formatVal(data.reduce((s, d) => s + d.sales, 0))}</span>
        </div>
        <div className="text-xs font-semibold text-neutral-500">
          <span className="text-neutral-400">Avg. daily</span>{" "}
          <span className="text-neutral-800">
            {formatVal(Math.round(data.reduce((s, d) => s + d.sales, 0) / Math.max(data.length, 1)))}
          </span>
        </div>
      </div>
    </DashboardCard>
