        ...
        <div className="flex flex-col items-center gap-6">
          <div className="relative mx-auto shrink-0" style={{ width: SIZE, height: SIZE }}>
            <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} className="-rotate-90" aria-hidden>
              ...
              <circle cx={CENTER} cy={CENTER} r={RADIUS} fill="none" stroke="#22c55e" strokeWidth={STROKE} strokeLinecap="round" strokeDasharray={`${fluentLength} ${CIRCUMFERENCE}`} />
              <circle cx={CENTER} cy={CENTER} r={RADIUS} fill="none" stroke="#f59e0b" strokeWidth={STROKE} strokeLinecap="round" strokeDasharray={`${congestedLength} ${CIRCUMFERENCE}`} strokeDashoffset={-fluentLength} />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
              <span className="text-3xl font-semibold tracking-tight text-neutral-900">{capacityPct}%</span>
              <span className="mt-1 text-xs font-semibold uppercase tracking-[0.24em] text-neutral-400">Active</span>
            </div>
          </div>

          <div className="w-full rounded-3xl border border-neutral-100 bg-neutral-50 p-4">
            <p className="text-xs font-semibold uppercase tracking-widest text-neutral-400">Order Status</p>
                  ...
                  return (
                    <div key={o.status}>
                      <div className="flex items-center justify-between text-sm font-semibold text-neutral-800">
                        ...
                        <span className={textColor}>{o.count} ({pct}%)</span>
                      </div>
